            ...
            try:
                ...
                if visitor.data:
                    ...
                    return User(
                        ...
                        device_fingerprint=v.get("device_fingerprint")  # Use get() to handle missing column
                    )
...
async def get_or_create_conversation_with_id(user_id: str, conversation_id: Union[str, None]):
    if conversation_id:
        return conversation_id
...
async def img2img(request: Request, file: UploadFile = File(...), prompt: str = ""):
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            files = {"image": (file.filename, content), "data": {"prompt": prompt, "n": 1, "size": "1024x1024"}}
...
async def chat_stream_endpoint(conversation_id: str, user_id: str, messages: list):
    ...
    async def event_generator():
        async for token_sse in stream_llm(user_id, conversation_id, messages):
            yield token_sse  # only yield here, no return here
...
async def multimodal_search_endpoint(request: MultimodalSearchRequest, req: Request, res: Response):
    ...
    return await multimodal_search(
        f"query: {request.query}\ntypes: {', '.join(request.search_types)}\nfilters: {filters_str}",
        user.id,
        False
    )
...
async def merge_user_data(req: Request, response: Response):
    ...
    try:
        ...
        if not frontend_supabase:
            raise HTTPException(401, "Frontend Supabase not configured")
        ...
        logger.error(f"Error in merge_user_data: {e}")
        raise HTTPException(500, f"Failed to merge user data: {str(e)}")

# Add the missing stop endpoint
@app.post("/stop")
async def stop_endpoint(request: Request):
    """Stop any active streams for a user"""
    user = await get_or_create_user(request, Response())
    user_id = user.id
...
# Add the missing user info endpoint
@app.get("/user/info")
async def get_user_info_endpoint(request: Request, response: Response):
...
# Add the missing /user/merge endpoint
@app.post("/user/merge")
async def merge_user_data_endpoint(request: Request, response: Response):
    ...
    auth_header = request.headers.get("authorization")
    if not auth_header or not auth_header.startswith("Bearer "):
        raise HTTPException(400, "No authorization token")
...
async def merge_user_data_endpoint(request: Request, response: Response):
    """Merge anonymous user data with authenticated user"""
    # This endpoint should be called after a user logs in
    # It merges the anonymous user's data with the user's data
...
# Add the missing /user/merge endpoint
@app.post("/user/merge")
async def merge_user_data_endpoint(request: Request):
    """Merge anonymous user data with authenticated user"""
    # This endpoint should be called after a user logs in
    # It merges the anonymous user's data with the user's data
...
async def get_user_info_endpoint(request: Request):
    ...
    try:
        ...
        # Get user's images count
        images_count = supjector.table("images").select("id", count="exact").eq("user_id", user_id).execute()
...
async def merge_user_data_endpoint(request: Request):
    """Merge anonymous user data with authenticated user"""
    # This endpoint should be called after a user logs in
    # It merges the user's anonymous data with the user's data
...
async def get_user_info_endpoint(request: Request):
    ...
    try:
        ...
        # Get user's images count
        images_count = supabase.table("files").select("id", count="exact").eq("user_id", user_id).execute()
        ...
        return {
            ...
            "stats": {
                "images": images_count.count if hasattr(images_count, 'count') else 0,
                "videos": videos_count.count if hasattr(videos_count, 'count') else 0,
                "conversations": conversations_count if hasattr(conversations_count, 'count') else 0
            }
        }
    except Exception as e:
        logger.error(f"Failed to get user info: {e}")
...
async def get_user_info_endpoint(request: Request):
    ...
    try:
        ...
        files_count = supabase.table("files").select("id", count="exact").eq("user_id", user_id).execute()
        ...
        return {
            ...
            "last_seen": user_data.get("last_seen") if user_data else None,
            "stats": {
                "files": files_count.count if hasattr(files_count, 'count') else 0,
                "videos": videos_count.count if hasattr(videos_count, 'count') else 0,
                "conversations": conversations_count if hasattr(conversations_count, 'count') else 0
            }
        }
...
async def merge_user_data_endpoint(request: Request):
    ...
    try:
        ...
        user_response = frontend_supabase.auth.get_user(token)
        if not user_response.user:
            raise HTTPException(1, "Invalid token")
...
async def merge_user_data_endpoint(request: Request):
    ...
    try:
        ...
        # Merge data in the backend
        try:
            # Update all records from anonymous to logged-in user
            tables_to_merge = ["images", "videos", "conversations", "messages", "memory", "vision_history", "code_generations"]
