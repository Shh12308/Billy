# app.py — Multi-provider AI Server with Supabase REST (NO Postgres direct connection)
...
from fastapi import (
    FastAPI, Request, HTTPException, Form, UploadFile, File, Header
)
...
def rate_limited(client_id: str):
    ...
    entries = [e for e in entries if e > now - RATE_LIMIT_WINDOW]
...
# ---------------------------- Provider (Mock) ----------------------------
async def provider_chat(model: str, messages, stream=False, params=None):
...
async def generate(req: GenerateRequest, request: Request, x_api_key: Optional[str] = Header(None)):
    ...
    out = await provider_chat(req.model or DEFAULT_MODEL, [{"role": "user", "content": req.prompt}], False)
