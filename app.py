...
SPHERE_TOKEN = os.getenv("SPHERE_TOKEN")
SPHERE_ENDPOINT = "https://compilers.sphere-engine.com/api/v4"
# Watermark settings
...
async def run_code_online(code: str, language: str):
    try:
        lang_id = LANGUAGE_MAP.get(language.lower(), 116)

        async with httpx.AsyncClient(timeout=60) as client:

            # Submit code
            submit = await client.post(
                f"{SPHERE_ENDPOINT}/submissions",
                params={"access_token": SPHERE_TOKEN},
                json={
                    "sourceCode": code,
                    "language": lang_id
                }
            ...
            result = await client.get(
                f"{SPHERE_ENDPOINT}/submissions/{submission_id}",
                params={"access_token": SPHERE_TOKEN}
            )
            ...
            return {
                ...
                "stderr": data.get("result", {}).get("stderr"),
                "time": data.get("result", {}).get("time"),
