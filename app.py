...
async def get_or_create_user(request: Request, response: Response) -> dict:
    ...
    user_data = {
        "id": anonymous_id,
        "email": f"anonymous+{anonymous_id}@zynara.local",
        "anonymous": True,
...
async def merge_anonymous_session(
    ...
    try:
        ...
        anon_resp = await run_in_threadpool(
            lambda: supabase.table("users")
            .select("id")
            .eq("session_token", session_token)
            .limit(1)
        ...
        for table in TABLES_WITH_USER:
            ...
            try:
                await run_in_threadpool(
                    ...
                    .update({"user_id": permanent_user_id})
...
async def run_code_online(code: str, language: str):
    ...
    try:
        suffix = FILE_SUFFIX.get(language, ".txt")
        ...
        if language == "c":
            exe = f"/tmp/{uuid.uuid4().hex}"
            compile_step = ["gcc", file_path, "-o", exe]
            cmd = [exe]

        elif language in ["cpp", "c++"]:
            exe = f"/tmp/{uuid.uuid4().hex}"
            compile_step = ["g++", file_path, "-o", exe]
            cmd = [exe]

        elif language == "rust":
            exe = f"/tmp/{uuid.uuid4().hex}"
            compile_step = ["rustc", file_path, "-o", exe]
            cmd = [exe]

        elif language == "java":
            compile_step = ["javac", file_path]
            classname = os.path.basename(file_path).replace(".java", "")
            ...
            cmd = LANGUAGE_COMMANDS[language] + [file_path]
...
async def ask_universal(
    ...
    # -------------------------
        ...
        return await handle_image(prompt, files, user_id, stream)

    # -------------------------
    # VIDEO GENERATION
    # -------------------------
    elif intent == "video":
        return await handle_video(prompt, user_id, stream)

    # -------------------------
    # MATH SOLVING
    # -------------------------
    elif intent == "math":
        return await handle_math(prompt, stream)

    # -------------------------
    # JOKE TELLING
    # -------------------------
    elif intent == "joke":
        return await handle_joke(prompt, stream)

    # -------------------------
    # PERSONAL INFO
    # -------------------------
    elif intent == "personal":
        return await handle_personal(prompt, user_id, stream)

    # -------------------------
    # VISION ANALYSIS
    # -------------------------
    elif intent == "vision" and files:
        return await handle_vision(request, files, stream)

    # -------------------------
    # IMG2VID
    # -------------------------
    elif intent == "img2vid" and files:
        return await handle_img2vid(request, files, prompt, stream)

    # -------------------------
    # CODE GENERATION
    # -------------------------
    elif intent == "code":
        return await handle_code(prompt, user_id, stream)

    # -------------------------
    # WEB SEARCH
    # -------------------------
    elif intent == "search":
        return await handle_search(prompt, stream)

    # -------------------------
    # TEXT-TO-SPEECH
    # -------------------------
    elif intent == "tts":
        return await handle_tts(prompt, stream)
    ...
    # -------------------------
        ...
        try:
            ...
            logger.error(f"Default chat processing failed: {e}")
            raise HTTPException(status_code=500, detail="Default chat processing failed")
